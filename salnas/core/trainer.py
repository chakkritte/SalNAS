...
def train_self_sd_dropout(student, optimizer, loader, epoch, device, args, scaler):
    student.train()
    tic = time.time()
    ...
    for idx, (img, gt, fixations) in enumerate(tqdm(loader)):
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            output, output_dp1, output_dp2 = student.forward_sd(img)
            loss = loss_func(output, gt, fixations, args)
            loss_kd1 = F.kl_div(output_dp1, output_dp2, reduction='mean')
            loss_kd2 = F.kl_div(output_dp2, output_dp1, reduction='mean')
            loss_kd = loss_kd1 + loss_kd2
            if epoch > 0:
                final_loss = loss + (0.1*loss_kd)  
            else:
                final_loss = loss 

        scaler.scale(final_loss).backward()
        total_loss += final_loss.item()
        cur_loss += final_loss.item()
