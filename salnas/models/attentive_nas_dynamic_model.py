...
class AttentiveNasDynamicModel(MyNetwork):
    ...
    def __init__(self, supernet, n_classes=1000, bn_param=(0., 1e-5), output_size = [480, 640]):
        super(AttentiveNasDynamicModel, self).__init__()

        self.supernet = supernet
        self.n_classes = n_classes
        self.use_v3_head = getattr(self.supernet, 'use_v3_head', True)
        ...
        self.ch_eml.append(out_channel_list)
        ...
        for stage_id, key in enumerate(self.stage_names[1:-1]):
            ...
            self.ch_eml.append(feature_dim)
        ...
        self.ch_eml.append(last_channel)
        #final conv layer
        ...
        self.active_resolution = 288

        self.decode_channel = [self.ch_eml[8], self.ch_eml[5], self.ch_eml[3], self.ch_eml[2], self.ch_eml[1], self.ch_eml[0]]
        # print(self.decode_channel)
        self.decode_output0 = self._make_output(self.decode_channel[0], self.decode_channel[1])
        self.decode_output1 = self._make_output([element * 2 for element in self.decode_channel[1]], self.decode_channel[2])
        self.decode_output2 = self._make_output([element * 2 for element in self.decode_channel[2]], self.decode_channel[3])
        self.decode_output3 = self._make_output([element * 2 for element in self.decode_channel[3]], self.decode_channel[4])
        self.decode_output4 = self._make_output([element * 2 for element in self.decode_channel[4]], self.decode_channel[5])
        self.decode_output5 = self._make_output_(self.decode_channel[5], self.decode_channel[5], 'swish', True)
        self.decode_output6 = self._make_output_(self.decode_channel[5], 1, 'sigmoid', True)
    ...
    def _make_output(self, planes, output):
        return nn.Sequential(
            DynamicConvBnActLayer(
                in_channel_list=int2list(planes), out_channel_list=int2list(output), 
                kernel_size=3, stride=1, act_func='swish', use_bn=True,
            ),
            nn.UpsamplingBilinear2d(scale_factor=2),
        )

    def _make_output_(self, planes, output, act, bn):
        return DynamicConvBnActLayer(
            in_channel_list=int2list(planes), out_channel_list=int2list(output), 
            kernel_size=3, stride=1, act_func=act, use_bn=bn,
        )
    ...
    def forward_once(self, x):
        ...
        if x.size(-1) != self.active_resolution:
            if self.active_resolution == 384:
                x = F.interpolate(x, size=(288, self.active_resolution), mode='bicubic')
            elif self.active_resolution == 256:
                x = F.interpolate(x, size=(192, self.active_resolution), mode='bicubic')
    ...
    def set_active_subnet(self, resolution=224, width=None, depth=None, kernel_size=None, expand_ratio=None, **kwargs):
        ...
        for stage_id, (c, k, e, d) in enumerate(zip(width[1:-1], kernel_size, expand_ratio, depth)):
            ...
            if stage_id == 4:
                self.decode_output0[0].active_out_channel = c
            elif stage_id == 2:
                self.decode_output1[0].active_out_channel = c
            elif stage_id == 1:
                self.decode_output2[0].active_out_channel = c
            elif stage_id == 0:
                self.decode_output3[0].active_out_channel = c

        self.decode_output4[0].active_out_channel = width[0] 
        self.decode_output5.active_out_channel = width[0]
        self.decode_output6.active_out_channel = 1
    ...
    def get_active_subnet_settings(self):
        ...
        if not self.use_v3_head:
            ...
            width.append(self.last_conv.final_expand_layer.active_out_channel)
    ...
    def get_active_subnet(self, preserve_weight=True):
        with torch.no_grad():
            ...
            for stage_id, block_idx in enumerate(self.block_group_info):
                ...
                blocks += stage_blocks
                ch_index.append(len(blocks)-1)
                ch_out.append(input_channel)
            ...
            ch_out.append(in_features)

            decode0 = nn.Sequential(
                self.decode_output0[0].get_active_subnet(ch_out[8], preserve_weight),
                nn.UpsamplingBilinear2d(scale_factor=2),
            )
            decode1 = nn.Sequential(
                self.decode_output1[0].get_active_subnet(ch_out[5]+ch_out[5], preserve_weight),
                nn.UpsamplingBilinear2d(scale_factor=2),
            )
            decode2 = nn.Sequential(
                self.decode_output2[0].get_active_subnet(ch_out[3]+ch_out[3], preserve_weight),
                nn.UpsamplingBilinear2d(scale_factor=2),
            )
            decode3 = nn.Sequential(
                self.decode_output3[0].get_active_subnet(ch_out[2]+ch_out[2], preserve_weight),
                nn.UpsamplingBilinear2d(scale_factor=2),
            )
            decode4 = nn.Sequential(
                self.decode_output4[0].get_active_subnet(ch_out[1]+first_conv.out_channels, preserve_weight),
                nn.UpsamplingBilinear2d(scale_factor=2),
            )
            
            decode5 = self.decode_output5.get_active_subnet(ch_out[0], preserve_weight)
            decode6 = self.decode_output6.get_active_subnet(ch_out[0], preserve_weight)

            ch_out_blocks = [decode0, decode1, decode2, decode3, decode4, decode5, decode6]

            # classifier = self.classifier.get_active_subnet(in_features, preserve_weight)

            _subnet = AttentiveNasStaticModel(
                first_conv, blocks, last_conv, None, self.active_resolution, use_v3_head=self.use_v3_head, ch_out_blocks=ch_out_blocks, ch_index=ch_index, output_size=self.output_size
            )
    ...
    def load_weights_from_pretrained_models(self, checkpoint_path):
        ...
        for k, v in self.state_dict().items():
            ...
            if "decode_" not in name:
                v.copy_(pretrained_state_dicts[name])

    def load_weights_from_pretrained_models_full(self, checkpoint_path):
        with open(checkpoint_path, 'rb') as f:
            checkpoint = torch.load(f, map_location='cpu')
        assert isinstance(checkpoint, dict)
        pretrained_state_dicts = checkpoint["student"]
        for k, v in self.state_dict().items():
            name = k
            v.copy_(pretrained_state_dicts[name])
