...
from salnas.core.trainer import train_pkd
...
parser.add_argument('--teacher',default="eeeac2", type=str)
...
parser.add_argument('--kd_mode', type=str, default="pkd", help='none | pkd | self')
...
args.save = '{}/{}-{}-pkd-{}'.format(args.output_dir, args.dataset, args.student, time.strftime("%Y%m%d-%H%M%S"))
...
params_group = [
    {"params": list(filter(lambda p: p.requires_grad, teacher.parameters())), "lr" : args.learning_rate*10 },
...
for epoch in range(0, args.no_epochs):
    ...
    loss = train_pkd(student, optimizer, train_loader, epoch, device, args, teacher, scaler)
    ...
    with torch.no_grad():
        ...
        if best_loss >= cc_loss:
            ...
            utils.save_state_dict_pkd(student, teacher, args)
        print()
